...
def aggregate_ticket_count(qs, flag: bool = None) -> int:
    if flag == None:
        qs_count = qs.aggregate(
            ticket_number=Count("branch_room__tenantroom__tenant_concern")
        )
    ...
    qs_count = qs.aggregate(
        ticket_number=Count(
            Case(
                When(branch_room__tenantroom__tenant_concern__is_answered=flag, then=1)
            )
        )
    )
    return qs_count["ticket_number"]
