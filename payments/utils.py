...
def calculate_payments(request, queryset, payment) -> T:
    ...
    total_payment = 0
    if queryset.count() != 0:
        total_payment = queryset.aggregate(
            remaining_balance=Sum("payment_received")
        )["remaining_balance"]
        if request.user.role == "T":
            payments = payment.objects.filter(tenant__tenant=request.user)             
        payments = payment.objects.filter(tenant__room__branch__assigned_landlord=request.user)     
