...
class Payment(BaseModel):
    ...
    tenant = models.ForeignKey(
        TenantRoom, on_delete=models.CASCADE, related_name="tenant_payments"
    )
    ...
    def number_of_months(self) -> Decimal:
        ...
        current_days_of_the_month = calendar.monthrange(
            self.end_date.year, self.end_date.month
        )[1]
        number_of_months = relativedelta.relativedelta(
            self.end_date, self.start_date
        ).months
        excess_days = (
            abs(self.end_date.day - self.start_date.day) / current_days_of_the_month
        )
...
class Transaction(BaseModel):
    transaction_no = models.CharField(
        max_length=15, unique=True, editable=False, default=generate_transation_number
    )
    payment_received = models.DecimalField(max_digits=8, decimal_places=2)
    date_received = models.DateField()
    payment = models.ForeignKey(
        Payment, on_delete=models.CASCADE, related_name="payment_transactions"
    )
    ...
    def remaining_balance(self) -> Decimal:
        total = (
            ...
            .aggregate(total=Sum("payment_received"))
        )
