...
class PaymentAPIView(generics.GenericAPIView, mixins.ListModelMixin):
    ...
    def get_queryset(self):
        queryset = Payment.objects.select_related("tenant").filter(
            tenant__room__branch__assigned_landlord=self.request.user
        )
...
class TransactionAPIView(generics.GenericAPIView, mixins.ListModelMixin):
    ...
    def get_queryset(self):
        ...
        queryset = None
        if user.role == "T":
            queryset = Transaction.objects.select_related("payment").filter(
                payment__tenant__tenant=user
            )
            return queryset

        queryset = Transaction.objects.select_related("payment").filter(
            payment__tenant__room__branch__assigned_landlord=self.request.user
        )
        return queryset
...
class RetrieveTransactionAPIView(
    generics.GenericAPIView, mixins.RetrieveModelMixin, DestroyInstanceMixin
):
