...
class PaymentSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def validate(self, attrs):
        ...
        logger.info(f"variables passed\nstart_date: {start_date}\nend_date: {end_date}\ntype: {type}")
        
        if (start_date is not None or end_date is not None) and type  == "Misc":
            raise serializers.ValidationError("both start_date and end_date fields should be empty if the payment type is 'Misc'")
        
        if (start_date is None or end_date is None) and type == "R":
            raise serializers.ValidationError("both start_date and end_date fields cannot be empty when the type is 'Rent'")
